...
def G14(s):
    ...
    model_path = os.path.join('/content/drive/My Drive/OpenShape/pointbert-vitg14-rgb','model.pt')
...
def L14(s):
    ...
    model_path = os.path.join('/content/drive/My Drive/OpenShape/pointbert-vitl14-rgb','model.pt')
    s = torch.load(model_path)
...
def B32(s):
    ...
    model_path = os.path.join('/content/drive/My Drive/OpenShape/pointbert-vitb32-rgb','model.pt')
    s = torch.load(model_path)
    dic = model.load_state_dict(module(s, 'pc_encoder'))
    print(dic)
    return model
